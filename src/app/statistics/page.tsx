        ...
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="grid grid-cols-4 gap-4">
            {[
              ...
              <div key={i} className="bg-white p-6 rounded-2xl border border-[#EBEBEA] shadow-sm">
                <div className="flex items-center justify-between mb-2">
                  <card.icon size={18} className={card.color} />
                </div>
                <div className="text-2xl font-bold">{card.value}</div>
                <div className="text-[12px] text-gray-500 uppercase tracking-wider font-medium">{card.label}</div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-3 gap-6">
            {/* Chart */}
            <div className="col-span-2 bg-white p-6 rounded-2xl border border-[#EBEBEA] shadow-sm">
                ...
                {['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс'].map((day, i) => {
                  ...
                  return (
                    <div key={day} className="flex flex-col items-center gap-2 w-full">
                      <div className="w-8 bg-indigo-500 rounded-t-lg transition-all duration-500" style={{ height: `${height}%`, minHeight: val > 0 ? '4px' : '0' }}></div>
            ...
            <div className="space-y-4">
              <div className="bg-white p-6 rounded-2xl border border-[#EBEBEA] shadow-sm">
                ...
                <div className="text-[12px] text-gray-500 uppercase tracking-wider font-medium">{t.bestStreak}</div>
              </div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-[#EBEBEA] shadow-sm">
                ...
                <div className="text-[12px] text-gray-500 uppercase tracking-wider font-medium">{t.coinsEarned}</div>
              </div>

              <div className="bg-white p-6 rounded-2xl border border-[#EBEBEA] shadow-sm">
                 <div className="text-[14px] font-bold mb-1">{stats?.avgMoves}</div>
                 <div className="text-[12px] text-gray-500 uppercase tracking-wider font-medium">{t.avgMoves}</div>
              </div>
            </div>
          </div>
        </div>
